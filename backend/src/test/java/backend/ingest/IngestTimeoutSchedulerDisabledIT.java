...
@TestPropertySource(properties = {
        "ingest.timeout.enabled=false",
        "ingest.timeout.stale-ms=1",
        "ingest.timeout.batch-size=100",
        "ingest.timeout.error-message=SHOULD_NOT_APPLY"
})
@Tag("it")
class IngestTimeoutSchedulerDisabledIT extends IntegrationTestBase {
    ...
    void sweep_whenDisabled_doesNothing() {
        long id = insertLinkResource("https://example.com/disabled", "example.com");
        jdbc.update(
